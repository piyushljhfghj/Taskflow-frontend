...
const Login = ({ onSubmit, onSwitchMode }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setOtpEmail(data.email); // redirect to OTP page
    } catch (err) {
  ...
  return (
    <div className="max-w-md w-full bg-white shadow-lg border border-purple-100 rounded-xl p-8">
      ...
      <div className="mt-4">
        <button onClick={handleGoogleLogin} className="w-full border border-gray-300 rounded-lg py-2 flex items-center justify-center gap-2">
          <FcGoogle /> Continue with Google
        </button>
      </div>
      <p className="text-center mt-4">
        Don't have an account? <button onClick={onSwitchMode} className="text-purple-600">Sign Up</button>
      </p>
    </div>
  );
};
